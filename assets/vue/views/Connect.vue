    ...
    >
        <div class="ui-auth">
            <ui-title class="ui-auth__title">
                {{ $t('module.settings.title_connection_setting') }}
            </ui-title>
            <div
                v-if="hasError"
                style="margin: 20px auto; max-width: 675px;"
            >
                <ui-alert
                    type="error"
                    icon="error"
                    ...
                    <form @submit.prevent="connect">
                        <div class="ui-auth-box__row">
                            ...
                            <ui-input
                                ...
                                required
                                type="url"
                                :placeholder="this.$t('module.connect.crm_url_placeholder')"
                                :error="errors.crmUrl"
                                @focus="addHttps"
                            />
                        </div>
                        <div class="ui-auth-box__row">
                            ...
                            <ui-input
                                v-model="dataCrmApiKey"
                                required
                                :error="errors.crmApiKey"
                            />
                        </div>
                        <div class="ui-auth-box__row ui-auth-box__row_btn">
                            <ui-button
                                :disabled="isLoading"
...
<script>
    export default {
        ...
        data() {
            return {
                dataCrmUrl: (this.crmUrl ? this.crmUrl : this.$route.query.account),
                dataCrmApiKey: this.crmApiKey,
            };
        },
        computed: {
            isLoading() {
                return this.$store.getters["connect/isLoading"];
            },
            hasError() {
                return this.$store.getters["connect/hasError"];
            },
            error() {
                return this.$store.getters["connect/error"];
            },
            errors() {
                return this.$store.getters["connect/errors"];
            },
        },
        methods: {
            async connect() {
                let data = {crmUrl: this.dataCrmUrl, crmApiKey: this.dataCrmApiKey};
                const result = await this.$store.dispatch("connect/connect", data);
                if (result !== null && result.redirectUrl) {
                    window.location.href = result.redirectUrl;
                }
            },
            addHttps() {
                if (!this.dataCrmUrl) {
                    this.dataCrmUrl = 'https://';
                }
            }
        }
    };
